...
from FILES.placeOrder_selectItems import placeOrder_selectItems

GROUPBOX_HEIGHT= 60

class placeOrder(QWidget):
    def __init__(self, main_database_path, vendors_database_path):
        ...
        self.layout.addWidget(self.refresh_button, 4, 0,1,-1)
    ...
    def initialise_lists(self):
        ...
        self.FILE.to_csv(self.vendors_database_path, index=False)

    def on_button_clicked(self):
        button= self.sender()
        index= self.buttons_list.index(button)
        self.vendor= index
        self.vendor_name= self.FILE.iloc[index][0]
        self.call_selectItems()

    def call_selectItems(self):
        self.window= placeOrder_selectItems(self.vendor, self.vendor, self.main_database_path, self.vendors_database_path)
        self.window.show()
